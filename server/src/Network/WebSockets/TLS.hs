...
runTLSServer
  ...
  -> IO ()
runTLSServer host port params app =
  runTLSServerWith host port params defaultConnectionOptions app
...
runTLSServerWith
  ...
  -> ServerApp
  -> IO ()
runTLSServerWith host port params opts app = do
 ...
 where
  runTLSApp con = do
    stream <- makeTLSStream con params
    pending <- makePendingConnectionFromStream stream opts
    app pending
